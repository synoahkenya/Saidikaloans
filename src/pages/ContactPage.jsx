        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
          ...
          <form onSubmit={handleSubmit} className="space-y-4 bg-gray-50 dark:bg-navy-light border border-gray-100 dark:border-navy-card rounded-3xl p-7">
            ...
            <div>
              ...
              <input type="tel" placeholder="0712 345 678" className="input-field" />
            </div>
            <div>
            ...
            <div>
              ...
              <select className="input-field">
                {['Loan Application','Repayment Issue','Account Problem','Credit Score Query','General Enquiry'].map((o) => <option key={o}>{o}</option>)}
              </select>
            </div>
            <div>
              <label className="text-sm font-semibold text-gray-600 dark:text-gray-400 block mb-1.5">Message</label>
              <textarea rows={4} placeholder="Tell us how we can help..." className="input-field resize-none" required />
            </div>
            <button type="submit" className="btn-primary w-full py-4 text-base rounded-2xl">Send Message →</button>
          </form>
        </div>
      </div>
    </div>
